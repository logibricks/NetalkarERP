    ...
    public class SalSoservice : ISalSoservice
    {
        ...
        public string Delete(int id, string cancellation_remarks, int reason_id)
        {
            try
            {
                ...
                var val = _scifferContext.Database.SqlQuery<string>(
                  "exec cancel_sales_invoice @id ,@cancellation_remarks ,@created_by,@cancellation_reason_id", pi_id, remarks, created_by, cancellation_reason_id).FirstOrDefault();
                return val;
            }
            catch (Exception ex)
            {
        ...
        public List<sal_so_vm> getall()
        {
            var val = _scifferContext.Database.SqlQuery<sal_so_vm>("get_so_index_list").ToList();
            return val;
